...
export const AddClientPage = ({ users, onBack, onSuccess }) => {
  const [form, setForm] = useState({
    ...
    tdsApplicable: false,
    ptMH: false, ptKA: false,
    itApplicable: false, auditCase: false,
    advanceTax: false,
    accounting: false,
  ...
  const eligibleUsers = users.filter(u => ['team_leader','executive','intern'].includes(u.role))

  // Estimate task count
  const fyS = parseInt(form.fy)
  const taskCount = [
    form.gstApplicable   && (form.gstFreq === 'monthly' ? 37 : 13), // 36/12 + annual
    form.tdsApplicable   && 16,
    form.ptMH            && 13,
    form.ptKA            && 2,
    form.itApplicable    && 1,
    form.advanceTax      && 4,
    form.accounting      && 12,
    form.caCertificates  && 1,
  ...
  const submit = async () => {
    ...
    try {
      const docRef  = await addClient(form)
  ...
  return (
    <div className="fade-up" style={{ padding: '24px 28px', maxWidth: 700 }}>
      ...
      <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 22 }}>
        Tasks for the entire financial year will be auto-created based on selected compliances.
      </div>

      {success && <Alert type="success" message="✓ Client onboarded! Tasks created successfully. Redirecting…" />}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div className="card" style={{ padding: 18 }}>
          ...
          <div className="grid-2" style={{ gap: 10 }}>
            ...
            <div><Label>TAN (for TDS)</Label><input placeholder="ABCD01234E" value={form.tan} onChange={e => set('tan', e.target.value.toUpperCase())} /></div>
            <div>
              <Label>Assign To *</Label>
              <select value={form.assignedTo} onChange={e => set('assignedTo', e.target.value)}>
                <option value="">-- Select Team Member --</option>
                {eligibleUsers.map(u => <option key={u.id} value={u.id}>{u.name} ({ROLES[u.role]})</option>)}
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ padding: 18 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <Toggle k="gstApplicable" label="GST (GSTR-1, GSTR-2B Reconciliation, GSTR-3B, GSTR-9)" />
            {form.gstApplicable && (
              <div style={{ background: 'var(--surface3)', borderRadius: 8, padding: '10px 12px', border: '1px solid var(--border2)', marginTop: -2 }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  {[['monthly', 'Monthly (11th)'], ['quarterly', 'Quarterly QRMP (13th)']].map(([v, l]) => (
                    <button key={v} onClick={() => set('gstFreq', v)}
                      className={`btn btn-sm ${form.gstFreq === v ? 'btn-primary' : 'btn-ghost'}`}>{l}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <Toggle k="ptMH"           label="Professional Tax – Maharashtra (Monthly payments + Annual return)" />
            <Toggle k="ptKA"           label="Professional Tax – Karnataka (Annual)" />
            ...
            {form.itApplicable && (
              <div style={{ background: 'var(--surface3)', borderRadius: 8, padding: '10px 12px', border: '1px solid var(--border2)', marginTop: -2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 13, color: 'var(--text)' }}>Audit Case? (Due: 31 Oct instead of 31 Jul)</span>
            ...
            <Toggle k="advanceTax"     label="Advance Tax (Quarterly instalments)" />
        ...
        {taskCount > 0 && (
          <div style={{ background: '#5b8dee15', border: '1px solid #5b8dee30', borderRadius: 10, padding: '12px 16px' }}>
            <div style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 700 }}>✓ Auto-generation preview</div>
            <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 4 }}>
              Approximately <strong style={{ color: 'var(--text)' }}>{taskCount} tasks</strong> will be created for FY {form.fy}
              {form.assignedTo && ` and assigned to ${users.find(u => u.id === form.assignedTo)?.name || 'selected member'}`}.
            </div>
          </div>
        )}

...
const ClientStatusControl = ({ client, currentUser }) => {
  ...
  const opts = [
    { v:'active',       l:'Active',       c:'#22c55e' },
    { v:'on_hold',      l:'On Hold',      c:'#f59e0b' },
    { v:'discontinued', l:'Discontinued', c:'#f43f5e' },
  ]
  return (
    <div style={{ display:'flex',alignItems:'center',gap:6,flexWrap:'wrap' }}>
      <span style={{ fontSize:11,color:'var(--text3)' }}>Client Status:</span>
      {opts.map(o=>(
        <button key={o.v} onClick={()=>change(o.v)} disabled={saving} style={{ padding:'4px 12px',borderRadius:20,fontSize:11,fontWeight:700,cursor:'pointer',border:`1px solid ${o.c}50`,transition:'all .15s',background:cur===o.v?`${o.c}20`:'transparent',color:cur===o.v?o.c:'var(--text3)',outline:cur===o.v?`1px solid ${o.c}`:'none' }}>{o.l}</button>
      ))}
    </div>
  )
}
...
const ClientRow = ({ client, users, tasks, onClick }) => {
  ...
  return (
    <div onClick={onClick} className="hover-lift" style={{ background:'var(--surface)',borderRadius:12,cursor:'pointer',border:`1px solid ${cst==='discontinued'?'#f43f5e40':cst==='on_hold'?'#f59e0b40':'var(--border)'}`,padding:'14px 18px',display:'grid',gridTemplateColumns:'2fr 1fr auto auto',alignItems:'center',gap:16,opacity:cst==='discontinued'?0.7:1 }}>
      <div>
      ...
      <div style={{ display:'flex',gap:16 }}>
        {[{v:overdue,l:'Overdue',c:'var(--danger)'},{v:active,l:'Active',c:'var(--warn)'},{v:done,l:'Done',c:'var(--success)'}].map(x=>(
          <div key={x.l} style={{ textAlign:'center' }}><div style={{ fontWeight:700,color:x.c,fontSize:16 }}>{x.v}</div><div style={{ fontSize:10,color:'var(--text3)' }}>{x.l}</div></div>
        ))}
      </div>
      ...
      <div className="card" style={{ padding:'12px 16px',marginBottom:16 }}>
        ...
        {cst!=='active' && (
          <div style={{ marginTop:8,fontSize:12,color:cst==='discontinued'?'var(--danger)':'var(--warn)' }}>
            {cst==='on_hold'?'⚠️ All tasks show ON HOLD badge. Internal statuses preserved.':'🛑 All tasks show STOP SERVICE badge. Client discontinued.'}
          </div>
        )}
      </div>
...
export const ClientsPage = ({ clients, users, tasks, currentUser, onAdd, onTask }) => {
  ...
  const live = selected ? clients.find(c=>c.id===selected.id)||selected : null
  if (live) return <ClientDetail client={live} tasks={tasks} users={users} currentUser={currentUser} onTask={onTask} onBack={()=>setSelected(null)}/>

  const counts = { all:clients.length, active:clients.filter(c=>(c.clientStatus||'active')==='active').length, on_hold:clients.filter(c=>c.clientStatus==='on_hold').length, discontinued:clients.filter(c=>c.clientStatus==='discontinued').length }

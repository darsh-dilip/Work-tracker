...
const ACTION_META = {
  ...
  arn_updated:           { l:'ARN Updated',           icon:'🔢', c:'#34d399' },
  ref_updated:           { l:'Reference Updated',     icon:'🔢', c:'#34d399' },
  comment_added:         { l:'Comment Added',         icon:'💬', c:'#38bdf8' },
  user_created:          { l:'User Created',          icon:'👤', c:'#a78bfa' },
  tasks_regenerated:     { l:'Tasks Regenerated',     icon:'⚙️',  c:'#fb923c' },
}
...
export const AuditLogPage = ({ users, clients }) => {
  ...
  const filtered = logs.filter(l => {
    ...
    if (search) {
      ...
      if (!`${l.entityName} ${l.clientName} ${l.by?.name} ${l.note}`.toLowerCase().includes(q)) return false
    }
    return true
  })
  ...
  return (
    <div className="fade-up" style={{ padding:'24px 28px', maxWidth:900 }}>
      ...
      <div style={{ fontSize:13, color:'var(--text2)', marginBottom:20 }}>
        Every action — status changes, reassignments, client updates — timestamped and attributed.
      </div>

      {/* Filters */}
      <div style={{ display:'grid', gridTemplateColumns:'1.5fr 1fr 1fr', gap:8, marginBottom:14 }}>
        <input placeholder="🔍  Search actions, clients, users…" value={search} onChange={e=>setSearch(e.target.value)}/>
                ...
                <div key={log.id} style={{
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                      ...
                      {log.clientName && (
                        <span style={{ fontSize:11, background:'#5b8dee15', color:'#5b8dee', padding:'1px 7px', borderRadius:10, border:'1px solid #5b8dee25' }}>
                          {log.clientName}
                        </span>
                      )}
                    </div>
                    <div style={{ fontSize:13, color:'var(--text)', marginBottom:3 }}>
                      ...
                      {log.oldValue && log.newValue && (
                        <span style={{ color:'var(--text2)' }}>
                          {' '}· <span style={{ color:'var(--text3)' }}>{log.oldValue}</span>
                          {' → '}<span style={{ color:meta.c }}>{log.newValue}</span>
                        </span>
                      )}
                    </div>
      ...
      {!loading && !filtered.length && (
        <div style={{ textAlign:'center', padding:40, color:'var(--text3)' }}>
          {logs.length ? 'No entries match your filters.' : 'No audit entries yet. Actions you take will appear here.'}
        </div>
      )}
    </div>
  )

...
import { getServiceStatuses, getStatusObj, COMMENT_STATUSES, LOG_ACTIONS, CLIENT_STATUS } from '../constants.js'
import { fmtDate } from '../utils/dates.js'
import { updateTask, addTaskComment } from '../hooks/useFirestore.js'
import { logTaskStatusChanged, logTaskReassigned, logCommentAdded } from '../utils/auditLog.js'
import { Modal, Label, Avatar, Divider, Alert } from './UI.jsx'
import { ROLES } from '../constants.js'

export const TaskModal = ({ task, users, clients, onClose, currentUser }) => {
  ...
  const [tab,         setTab]         = useState('status') // 'status' | 'comments' | 'history'

  const assignee    = users.find(u => u.id === task.assignedTo)
  const client      = clients?.find(c => c.id === task.clientId)
  const selectedSt  = statuses.find(s => s.v === status) || {}
  const clientSt    = client?.clientStatus || 'active'
  const clientBadge = CLIENT_STATUS[clientSt]
  ...
  const save = async () => {
    ...
    try {
      ...
      const updates = {
        ...
        history: arrayUnion({
          action:  `Status → ${selectedSt.l || status}`,
          by:      currentUser.id,
      ...
      await logTaskStatusChanged(task, oldStatus, status, currentUser, arn || ref)
      onClose()
  ...
  const doReassign = async () => {
    const oldName = assignee?.name || 'Unknown'
    const newName = users.find(u => u.id === newAssignee)?.name || 'Unknown'
    await updateTask(task.id, {
      assignedTo: newAssignee,
      history:    arrayUnion({ action: `Reassigned → ${newName}`, by: currentUser.id, byName: currentUser.name, at: new Date().toISOString() }),
    })
    await logTaskReassigned(task, oldName, newName, currentUser)
  ...
  return (
    <Modal open onClose={onClose} title={`${task.service}`} width={580}>
      {/* Client-level badge */}
      {clientBadge?.badge && (
        <div style={{ background: clientBadge.badgeBg, border: `1px solid ${clientBadge.badgeBorder}`, borderRadius: 8, padding: '8px 14px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontWeight: 800, fontSize: 12, color: clientBadge.badgeColor, letterSpacing: '0.08em' }}>{clientBadge.badge}</span>
          <span style={{ fontSize: 12, color: clientBadge.badgeColor }}>{client?.name} is currently {CLIENT_STATUS[clientSt]?.l?.toLowerCase()}</span>
        </div>
      )}
      ...
      <div style={{ background: 'var(--surface2)', borderRadius: 10, padding: '12px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
        <div><Label>Client</Label><div style={{ fontWeight: 600, fontSize: 13, color: 'var(--text)' }}>{task.clientName}</div></div>
        <div><Label>Period</Label><div style={{ fontSize: 13, color: 'var(--text)' }}>{task.period}</div></div>
        <div><Label>Due Date</Label><div style={{ fontSize: 13, color: 'var(--text)' }}>{fmtDate(task.dueDate)}</div></div>
        <div>
          <Label>Assigned To</Label>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {assignee && <Avatar name={assignee.name} init={assignee.init} role={assignee.role} sz={20} />}
            <span style={{ fontSize: 13, color: 'var(--text)' }}>{assignee?.name || 'Unassigned'}</span>
          </div>
        </div>
        <div>
          <Label>Current Status</Label>
          <span className="chip" style={{ background: currentSt.bg, color: currentSt.c, border: `1px solid ${currentSt.c}35` }}>{currentSt.l || task.status}</span>
        </div>
        {(task.arn || task.ref) && (
          <div>
            <Label>{task.arn ? 'ARN / Ack No.' : 'Reference'}</Label>
            <div style={{ fontSize: 12, color: 'var(--success)', fontFamily: 'var(--mono)' }}>{task.arn || task.ref}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, borderBottom: '1px solid var(--border)', paddingBottom: 10 }}>
        {[['status','✏️ Update Status'],['comments','💬 Comments'],['history','📋 History']].map(([t,l]) => (
          <button key={t} onClick={() => setTab(t)} style={tabStyle(tab === t)}>{l}</button>
        ))}
        <button onClick={() => setReassigning(!reassigning)} style={{ ...tabStyle(reassigning), marginLeft: 'auto' }}>↔ Reassign</button>
      </div>
      ...
      {tab === 'status' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            ...
            {/* Show colour preview of selected status */}
            {selectedSt.l && (
              <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {selectedSt.done && <span style={{ fontSize: 11, color: 'var(--success)' }}>✓ This marks the task as done</span>}
              </div>
            )}
          </div>
          ...
          {selectedSt.requiresRef && (
            <div>
              <Label>{selectedSt.refLabel || 'Reference Number'} *</Label>
              <input placeholder={`Enter ${selectedSt.refLabel || 'reference'}`} value={ref} onChange={e => setRef(e.target.value)} />
            </div>
          )}

          <div>
            <Label>Internal Note / Remark (optional)</Label>
            <textarea placeholder="Add an internal note…" value={note} onChange={e => setNote(e.target.value)} rows={2} style={{ resize: 'vertical' }} />
          </div>
      ...
      {tab === 'comments' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* Quick-tag from Comment Statuses */}
          <div>
            <Label>Quick Tag (from Comment Status list)</Label>
            ...
            <div key={i} style={{ display: 'flex', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                  by {h.byName || '?'} · {new Date(h.at).toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })}
                </div>
              </div>
            </div>
          ))}

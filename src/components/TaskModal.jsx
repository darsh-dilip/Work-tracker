...
export const TaskModal = ({ task, users, clients, currentUser, onClose }) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      await updateTask(task.id, {
        ...
        history: arrayUnion({ action:`Status → ${selectedSt.l||status}`, by:currentUser.id, byName:currentUser.name, at:new Date().toISOString(), oldStatus }),
      })
    ...
    <Modal open onClose={onClose} title={task.service} width={580}>
      {clientBadge?.badge && (
        <div style={{ background:clientBadge.badgeBg,border:`1px solid ${clientBadge.badgeBorder}`,borderRadius:8,padding:'8px 14px',marginBottom:14,display:'flex',alignItems:'center',gap:10 }}>
          ...
          <span style={{ fontSize:12,color:clientBadge.badgeColor }}>{client?.name} is {CLIENT_STATUS[clientSt]?.l?.toLowerCase()}</span>
        </div>
      )}
      ...
      <div style={{ background:'var(--surface2)',borderRadius:10,padding:'12px 14px',display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:16 }}>
        ...
        {(task.arn||task.ref) && (
          <div><Label>{task.arn?'ARN / Ack':'Reference'}</Label><div style={{ fontSize:12,color:'var(--success)',fontFamily:'var(--mono)' }}>{task.arn||task.ref}</div></div>
        )}
      </div>
      ...
      {tab==='status' && (
        <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
          ...
          {selectedSt.requiresArn && (
            <div><Label>ARN / Acknowledgement Number *</Label><input placeholder="Enter ARN or Ack. No." value={arn} onChange={e=>setArn(e.target.value)}/></div>
          )}

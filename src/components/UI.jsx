...
import { ROLE_CLR, DONE_STATUSES, CLIENT_STATUS } from '../constants.js'
import { getStatusObj } from '../constants.js'
...
export const Avatar = ({ name, init, role, sz=32 }) => {
  ...
  return (
    <div style={{ width:sz,height:sz,borderRadius:'50%',background:`${c}20`,border:`1.5px solid ${c}45`,
      display:'flex',alignItems:'center',justifyContent:'center',fontSize:sz*.36,fontWeight:700,color:c,flexShrink:0 }}>
      {i}
    </div>
  )
}

/* ── Status Badge (service-aware) ── */
export const StatusBadge = ({ service, status }) => {
  const s = getStatusObj(service || '', status)
  return <span className="chip" style={{ background:s.bg,color:s.c,border:`1px solid ${s.c}35` }}>{s.l}</span>
}

/* ── Due Badge ── */
export const DueBadge = ({ dueDate, status, service }) => {
...
const ClientBadgeOverlay = ({ clientStatus }) => {
  ...
  return (
    <span style={{ display:'inline-flex',alignItems:'center',padding:'2px 7px',borderRadius:4,
      fontSize:9,fontWeight:800,letterSpacing:'0.07em',background:st.badgeBg,
      color:st.badgeColor,border:`1px solid ${st.badgeBorder}`,flexShrink:0 }}>
      {st.badge}
    </span>
  )
}
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.72)',zIndex:1000,
      ...
      <div onClick={e=>e.stopPropagation()} className="fade-up" style={{
        ...
        <div style={{ padding:'18px 22px',borderBottom:'1px solid var(--border)',display:'flex',
          alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,background:'var(--surface)',zIndex:1 }}>
...
export const Alert = ({ type='error', message }) => {
  ...
  const s = { error:{bg:'#f43f5e15',border:'#f43f5e30',color:'var(--danger)'},
               success:{bg:'#22c55e15',border:'#22c55e30',color:'var(--success)'},
               info:{bg:'#5b8dee15',border:'#5b8dee30',color:'var(--accent)'} }[type] || {}
  return <div style={{ background:s.bg,border:`1px solid ${s.border}`,borderRadius:8,padding:'10px 12px',color:s.color,fontSize:13 }}>{message}</div>
}
...
/* ── TaskRow — now shows client overlay badge ── */
export const TaskRow = ({ task, users, clients, onClick }) => {
  const assignee   = users.find(u => u.id === task.assignedTo)
  const client     = clients?.find(c => c.id === task.clientId)
  const clientSt   = client?.clientStatus || 'active'
  const isDone     = DONE_STATUSES.includes(task.status)
  const statusObj  = getStatusObj(task.service, task.status)

  return (
    <div onClick={onClick} className="hover-lift" style={{
      display:'grid',gridTemplateColumns:'2fr 1.2fr 1fr 150px 110px 20px',
      alignItems:'center',gap:12,padding:'11px 16px',
      background:'var(--surface2)',borderRadius:10,cursor:'pointer',
      border:`1px solid ${clientSt==='discontinued'?'#f43f5e30':clientSt==='on_hold'?'#f59e0b30':'var(--border)'}`,
      opacity:isDone?0.65:1,marginBottom:4,
    }}>
      <div>
        <div style={{ display:'flex',alignItems:'center',gap:7,marginBottom:2 }}>
          <span style={{ fontWeight:600,fontSize:13,color:'var(--text)' }}>{task.service}</span>
          {clientSt!=='active' && <ClientBadgeOverlay clientStatus={clientSt}/>}
        </div>
      ...
      <span className="chip" style={{ background:statusObj.bg,color:statusObj.c,border:`1px solid ${statusObj.c}35` }}>{statusObj.l}</span>

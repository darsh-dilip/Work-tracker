...
import { ROLE_CLR, DONE_STATUSES, CLIENT_STATUS, getStatusObj } from '../constants.js'
...
export const Alert = ({ type='error', message }) => {
  ...
  const m = {error:{bg:'#f43f5e15',border:'#f43f5e30',color:'var(--danger)'},success:{bg:'#22c55e15',border:'#22c55e30',color:'var(--success)'},info:{bg:'#5b8dee15',border:'#5b8dee30',color:'var(--accent)'}}
...
export const TaskRow = ({ task, users, clients, onClick }) => {
  ...
  const bdrColor  = clientSt==='discontinued'?'#f43f5e30':clientSt==='on_hold'?'#f59e0b30':'var(--border)'
  return (
    <div onClick={onClick} className="hover-lift" style={{ display:'grid',gridTemplateColumns:'2fr 1.2fr 1fr 150px 110px 20px',alignItems:'center',gap:12,padding:'11px 16px',background:'var(--surface2)',borderRadius:10,cursor:'pointer',border:`1px solid ${bdrColor}`,opacity:isDone?.65:1,marginBottom:4 }}>
      <div>
        <div style={{ display:'flex',alignItems:'center',gap:7,marginBottom:2 }}>
          <span style={{ fontWeight:600,fontSize:13,color:'var(--text)' }}>{task.service}</span>
          {clientSt!=='active' && <ClientOverlay clientStatus={clientSt}/>}
        </div>

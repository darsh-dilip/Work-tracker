...
export default function App() {
  ...
  if (usersLoading || clientsLoading || tasksLoading) return (
    <div style={{ minHeight:'100vh',background:'var(--bg)',display:'flex',alignItems:'center',justifyContent:'center' }}>
  ...
  const renderPage = () => {
    switch (page) {
      case 'dashboard':
        return <DashboardPage tasks={tasks} user={userProfile} users={users} clients={clients} onTask={setSelectedTask}/>
      case 'tasks':
        return <TasksPage tasks={tasks} user={userProfile} users={users} clients={clients} onTask={setSelectedTask}/>
      case 'clients':
        return <ClientsPage clients={clients} users={users} tasks={tasks} currentUser={userProfile}
          onAdd={()=>setPage('add_client')} onTask={setSelectedTask}/>
      case 'add_client':
        return <AddClientPage users={users} onBack={()=>setPage('clients')} onSuccess={()=>setPage('clients')} currentUser={userProfile}/>
      case 'workload':
        return <DashboardWorkload users={users} tasks={tasks} clients={clients} user={userProfile} onTask={setSelectedTask}/>
      case 'gst':
        return <DashboardGST clients={clients} tasks={tasks} users={users} onTask={setSelectedTask}/>
      case 'it':
        return <DashboardIT clients={clients} tasks={tasks} users={users} onTask={setSelectedTask}/>
      case 'audit':
        return <AuditLogPage users={users} clients={clients}/>
      case 'users':
